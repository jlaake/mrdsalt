...
#' @param persons a dataframe containing a single record per observation and 2 columns indicating which person was in observer (position) 1 and 2 which provides the opportunity to estimate of personnel effects on detection
...
make.design.data=function(x,persons=NULL)
{
  ...
  if(!is.null(persons))
  {
    if(is.data.frame(persons) & ncol(persons)==2&nrow(persons)==nrow(x))
    {
      dd$person=as.vector(t(persons))
