...
summary.loglinear=function (object, se = TRUE, ...)
{
  model <- object
  ...
  ans <- list(par=model$par, vc=solve(model$hessian), Nhat = model$Nhat, AIC = model$criterion,
              average.p = n/model$Nhat, mono = FALSE, mono.strict = FALSE)
