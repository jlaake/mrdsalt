...
#' #fit model
#' results=ddf(mrmodel=list(pformula=~observer*distance,dformula=~-1+distance),data=dd,method="loglinear",meta.data=list(width=W),control=list(method="nlminb",debug=FALSE))
#' #N hat
...
ddf.loglinear=function(mrmodel=list(pformula=~-1+observer+observer:distance,dformula=~-1+distance),data,meta.data=meta.data,control=control,call="")
{
  ...
  if(!is.na(control$initial))par=control$initial
