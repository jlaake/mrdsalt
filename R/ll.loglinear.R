...
ll.loglinear=function(par,cmat,dd,W,pformula,dformula,debug)
{
  ...
  {
    esw=integrate(mu,lower=0,upper=W,par=par,dd=dd[1:2,],pformula=pformula,dformula=dformula)$value
  ...
  } else
  {
    ...
    for(i in 1:n)
      nll=nll+log(integrate(mu,lower=0,upper=W,par=par,dd=dd[((i-1)*2+1):(i*2),],pformula=pformula,dformula=dformula)$value)
  }
